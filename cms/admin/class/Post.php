...
class Post
{
	...
	public function getPostsListing()
	{
		...
		while ($post = $result->fetch_assoc()) {
			...
			$rows[] = $post['created'];
			$rows[] = $post['updated'];
			$rows[] = '<a href="edit_post.php?id=' . $post["id"] . '" class="btn btn-warning btn-xs update">Editar</a>';
			$rows[] = '<button type="button" name="delete" id="' . $post["id"] . '" class="btn btn-danger btn-xs delete">Eliminar</button>';
			$posts[] = $rows;
		}
